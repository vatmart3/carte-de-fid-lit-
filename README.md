...
| **Réglages** | Nom / ville / accroche de la boutique, barème (points par euro, bienvenue, anniversaire, parrain, filleul), verrouillage automatique, éditeur de récompenses, compte d'accès, export CSV clients et achats, sauvegarde et restauration JSON. **Logo** : celui de la boutique est inscrit dans le fichier (`assets/logo/vatuone.png`, 11 Ko en data URI) et sert par défaut — en couleurs dans le bandeau, découpé au pochoir CSS sur les cartes, où il prend l'encre du style ; un fichier déposé le remplace, réencodé en PNG 384 px (un SVG est du code, redessiné il n'en reste qu'une image), recadré sur son dessin à pleine résolution avant réduction (un logo d'imprimeur est souvent seul au milieu d'une page blanche), essayé sur fond clair et sombre, option « logo clair » qui lui pose une plaque foncée. **Points en plus** : trois actions (Instagram, Facebook, avis Google) avec leur lien et leur barème — une action ayant lien et points est proposée, rien à activer, et chaque ligne affiche son état ; le client les déclenche depuis sa carte, une seule fois chacune — l'unicité (index partiel) et le nombre de points (lu dans la fiche boutique) sont tenus par la base, jamais par la page. |
