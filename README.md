...
| **Réglages** | Nom / ville / accroche de la boutique, barème (points par euro, bienvenue, anniversaire, parrain, filleul), verrouillage automatique, éditeur de récompenses, compte d'accès, export CSV clients et achats, sauvegarde et restauration JSON. **Logo** : le fichier de la boutique remplace l'initiale du bandeau et le fer des cartes ; réencodé en PNG 384 px (un SVG est du code, redessiné il n'en reste qu'une image), essayé sur fond clair et sombre, option « logo clair » qui lui pose une plaque foncée. |
